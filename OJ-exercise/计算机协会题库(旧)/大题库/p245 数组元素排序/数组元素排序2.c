...
int main()
{
    ...
    int num = 10000;            //定义最大值存储变量num
    int num2;               //定义最大数位于输入数组arr[]的位置
    int x = 0;              //定义输出数组arr2[]的各个位数
    ...
    for(int m=n-1;m>=0;m--,x++)    //将输入数组arr[]中最大值依次存入输出数组arr2[]循环
    {
    ...
    arr[num2] = 10000;          //每次将输入数组arr[]最大值删去
    num = 10000;                //将每次最大值num归零
